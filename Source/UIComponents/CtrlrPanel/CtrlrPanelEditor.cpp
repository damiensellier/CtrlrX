    ...
    canvasHeight(0)
{
    ...
    // On macOS, default to true only if the running app has the correct JIT entitlements, otherwise false
        setProperty(Ids::uiPanelJitMode, isAppSignedWithEntitlements());  // Added v5.6.36. Set LuaJIT to Interpretor only OR Full mode with compilator (requires macOS deep codesigning to prevent crash)
    }
    else {
...
//            LookAndFeel::setDefaultLookAndFeel(customLookAndFeel);
		...
		{
			// Retrieve the lua_State using your panel's Lua manager pattern
			...
			if (L != nullptr)
			{
				...
				if (jitRequested)
				{
					...
					if (!isAppSignedWithEntitlements())
					{
						...
						String warningMsg = "Cannot enable JIT. Missing 'com.apple.security.cs.allow-unsigned-executable-memory' entitlement on this macOS build.";
						...
						// verify if the instance is restricted to prevent updating the value and then changing the binary
						bool isRestricted = owner.getOwner().getInstanceMode() == InstanceSingleRestricted;
						if (isRestricted){
							_DBG("instance is restricted, uiPanelJitMode value not updated");
						}
						else {
							setProperty(Ids::uiPanelJitMode, false); // Resets the property to false
							getPropertiesPanel()->refreshAll(); // refreshes the property panel
							if (getSelection())
							{
								getSelection()->sendChangeMessage();  // Bring back the screen position to where it left
							}
							_DBG("Instance is not restricted, uiPanelJitMode value updated");
						}
					}
					else
					{
...
#else
					// Windows / Linux don't require this entitlement restriction
					luaL_dostring(L, "jit.on()");
#endif
				}
				else
				...
				lua_pop(L, 2); // Clean up stack (pop result and 'jit' table)
...
bool CtrlrPanelEditor::isAppSignedWithEntitlements()
{
    ...
    if (SecCodeCopySigningInformation(selfCode, kSecCSDefaultFlags, &info) == errSecSuccess && info)
    {
        ...
        else
        {
            ...
            {
                entitlements = (CFDictionaryRef)CFPropertyListCreateWithData(
                    kCFAllocatorDefault, (CFDataRef)dataObj, kCFPropertyListImmutable, NULL, NULL);
            }
        }

        if (entitlements && CFGetTypeID(entitlements) == CFDictionaryGetTypeID())
        {
            CFBooleanRef allowUnsignedMem = (CFBooleanRef)CFDictionaryGetValue(entitlements, CFSTR("com.apple.security.cs.allow-unsigned-executable-memory"));
